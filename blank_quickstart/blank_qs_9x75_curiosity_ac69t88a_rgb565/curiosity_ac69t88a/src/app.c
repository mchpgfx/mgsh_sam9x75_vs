...
/* Color Palette Lookup Table
  ...
  Summary:
    Predefined color palette in RGB565 format.
  ...
  Remarks:
    Colors are stored as 16-bit RGB565 values (RRRRRGGGGGGBBBBB format).
*/
uint16_t color_map[] =
    {
...
/*******************************************************************************
  Function:
    void draw_line(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint16_t color)
...
/*******************************************************************************
  Function:
    void draw_circle(uint32_t x, uint32_t y, uint32_t r, uint16_t color, bool fill)
...
/*******************************************************************************
  Function:
    void draw_rect_fill(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint16_t color)
  ...
  Parameters:
    ...
    color - 16-bit RGB565 fill color value
...
/*******************************************************************************
  Function:
    uint32_t draw_char(uint32_t x, uint32_t y, char ascii_char, font_size_alias_t font_size, uint16_t color)
  ...
  Parameters:
    ...
    color      - 16-bit RGB565 color value for the character
...
/*******************************************************************************
  Function:
    uint32_t draw_string(uint32_t x, uint32_t y, const char *str, font_size_alias_t font_size, uint16_t color)
  ...
  Parameters:
    ...
    font_size - Font size selector (FONT_SM, FONT_BASE, etc.)
    color     - 16-bit RGB565 color value for the text
...
/*******************************************************************************
  Function:
    uint16_t get_gradient(uint16_t start_color, uint16_t end_color, uint32_t current_step, uint32_t total_steps)
  ...
  Description:
    ...
    position within a gradient. Each color channel (R5, G6, B5) is
    interpolated independently.

  Parameters:
    start_color  - Starting color (16-bit RGB565)
    end_color    - Ending color (16-bit RGB565)
    current_step - Current position in the gradient (0 to total_steps)
    total_steps  - Total number of steps in the gradient

  Returns:
    Interpolated 16-bit RGB565 color value.
*******************************************************************************/
uint16_t get_gradient(uint16_t start_color, uint16_t end_color, uint32_t current_step, uint32_t total_steps)
{
    ...
    uint8_t r = start_r + (end_r - start_r) * current_step / total_steps;
    uint8_t g = start_g + (end_g - start_g) * current_step / total_steps;
    uint8_t b = start_b + (end_b - start_b) * current_step / total_steps;
...
void draw_test_pattern(uint32_t width, uint32_t height)
{
    ...
    for (int i = 2; i < x_div - 2; i++)
    {
        ...
        draw_string(cx + 4, y_offset * 4 + 4, tmp, FONT_SM, (i < 7) ? 0xFFFF : 0x0000);
    }
